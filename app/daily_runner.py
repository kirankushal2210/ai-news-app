    ...
    try:
        ...
        try:
            ...
            if email_result["success"]:
                ...
                logger.warning(f"⚠️  Email sending skipped: {email_result.get('error', 'Unknown error')}")
                results["success"] = True  # Mark as success even if email fails (digests were created)
        except Exception as e:
            logger.warning(f"⚠️  Email sending skipped (not configured): {e}")
            results["success"] = True  # Digests were created successfully
